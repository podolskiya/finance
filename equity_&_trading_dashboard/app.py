...
if "nav" in st.query_params:
    st.session_state.current_page = st.query_params["nav"]
...
with header_cols[0]:
    ...
    logo_html = f"""
    <a href="/?nav=Overview" target="_self">
        <img src="data:image/svg+xml;base64,{base64_logo}" width="40" style="cursor: pointer;" alt="Home">
    </a>
    """
...
with header_cols[2]:
    with st.popover("MARKET ANALYSIS"):
        ...
        if st.button("Model Analyser", use_container_width=True):
            st.session_state.current_page = "Model Analyser"; st.rerun()
...
elif page_selection == "Model Analyser":
    from app.pages import model_upload; model_upload.show()